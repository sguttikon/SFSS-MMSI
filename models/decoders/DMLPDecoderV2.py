...
class DMLPv2(nn.Module):
    def __init__(self,
        ...
        self.head1 = DeformableMLPBlock(in_chans=c1_in_channels, emb_chans=embedding_dim)
        self.head2 = DeformableMLPBlock(in_chans=c2_in_channels, emb_chans=embedding_dim)
        self.head3 = DeformableMLPBlock(in_chans=c3_in_channels, emb_chans=embedding_dim)
        self.head4 = DeformableMLPBlock(in_chans=c4_in_channels, emb_chans=embedding_dim)
    ...
    def forward(self, inputs):
        ...
        c4 = self.head4(c4)
        c4 = F.interpolate(c4, size=c1.size()[2:], mode='bilinear', align_corners=self.align_corners)

        c3 = self.head3(c3)
        c3 = F.interpolate(c3, size=c1.size()[2:], mode='bilinear', align_corners=self.align_corners)

        c2 = self.head2(c2)
        c2 = F.interpolate(c2, size=c1.size()[2:], mode='bilinear', align_corners=self.align_corners)

        c1 = self.head1(c1)

        out = self.linear_fuse(torch.cat([c1, c2, c3, c4], dim=1))
