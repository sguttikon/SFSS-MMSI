...
class Evaluator(object):
    def __init__(self, data_pipeline, rgb, ann, class_num, class_names, ignore_index, modality_x,
                eval_crop_size, eval_stride_rate,
                norm_mean, norm_std, network, multi_scales, 
                is_flip, devices, verbose=False, save_path=None, show_image=False):
        self.eval_time = 0
        self.data_pipeline = data_pipeline
        self.ndata = len(self.data_pipeline)
        self.rgb = rgb
        self.ann = ann
        self.class_num = class_num
        self.class_names = class_names
        self.ignore_index = ignore_index
        self.modality_x = modality_x
        self.eval_crop_size = eval_crop_size
        self.eval_stride_rate = eval_stride_rate
        ...
        self.valid_labels = np.arange(self.class_num).tolist() + [self.ignore_index]
    ...
    def run(self, model_path, model_indice, log_file, log_file_link):
        ...
        if '.pth' in model_indice:
            models = [os.path.join(model_path, model_indice), ]
        ...
        for model in models:
            ...
            if len(self.devices ) == 1:
                ...
                # x = torch.zeros(1, 3, 512, 512).to(self.devices[0])
                # logger.info(flop_count_table(FlopCountAnalysis(self.val_func, x))) 
                iou_result = self.single_process_evalutation()
            else:
                iou_result = self.multi_process_evaluation()

            iou_results.append(iou_result)
            result_line = print_iou(iou_result['IoU'], 0,
                                    np.nanmean(iou_result['Acc']), iou_result['aAcc'],
    ...
    def single_process_evalutation(self):
        ...
        all_results = []
        for idx, dd in tqdm(enumerate(self.data_pipeline)):
            results_dict = self.func_per_iteration(dd,self.devices[0])
            all_results.append(results_dict)
        iou_result = self.compute_metric(all_results)
        logger.info(
    ...
    def sliding_eval_rgbX(self, img, modal_x1, modal_x2, crop_size, stride_rate, device=None):
        ...
        for s in self.multi_scales:
            ...
            if len(modal_x1.shape) == 2:
                modal_x1_scale = cv2.resize(modal_x1, None, fx=s, fy=s, interpolation=cv2.INTER_NEAREST)
            else:
                modal_x1_scale = cv2.resize(modal_x1, None, fx=s, fy=s, interpolation=cv2.INTER_LINEAR)
            if len(modal_x2.shape) == 2:
                modal_x2_scale = cv2.resize(modal_x2, None, fx=s, fy=s, interpolation=cv2.INTER_NEAREST)
            else:
                modal_x2_scale = cv2.resize(modal_x2, None, fx=s, fy=s, interpolation=cv2.INTER_LINEAR)

            new_rows, new_cols, _ = img_scale.shape
            processed_pred += self.scale_process_rgbX(img_scale, modal_x1_scale, modal_x2_scale,
                                                      (ori_rows, ori_cols),
                                                      crop_size, stride_rate, device)
    ...
    def scale_process_rgbX(self, img, modal_x1, modal_x2, ori_shape, crop_size, stride_rate, device=None):
        new_rows, new_cols, c = img.shape
        long_size = new_cols if new_cols > new_rows else new_rows

        if new_cols <= crop_size[1] or new_rows <= crop_size[0]:
            input_data, input_modal_x1, input_modal_x2, margin = self.process_image_rgbX(img, modal_x1, modal_x2, crop_size)
            score = self.val_func_process_rgbX(input_data, input_modal_x1, input_modal_x2, device) 
            ...
            modal_x_pad1, margin = pad_image_to_shape(modal_x1, crop_size, cv2.BORDER_CONSTANT, value=0)
            modal_x_pad2, margin = pad_image_to_shape(modal_x2, crop_size, cv2.BORDER_CONSTANT, value=0)
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    s_y = e_y - crop_size[1]
                    img_sub = img_pad[s_y:e_y, s_x: e_x, :]
                    if len(modal_x_pad1.shape) == 2:
                        modal_x_sub1 = modal_x_pad1[s_y:e_y, s_x: e_x]
                    else:
                        modal_x_sub1 = modal_x_pad1[s_y:e_y, s_x: e_x,:]
                    if len(modal_x_pad2.shape) == 2:
                        modal_x_sub2 = modal_x_pad2[s_y:e_y, s_x: e_x]
                    else:
                        modal_x_sub2 = modal_x_pad2[s_y:e_y, s_x: e_x,:]

                    input_data, input_modal_x1, input_modal_x2, tmargin = self.process_image_rgbX(img_sub, modal_x_sub1, modal_x_sub2, crop_size)
                    temp_score = self.val_func_process_rgbX(input_data, input_modal_x1, input_modal_x2, device)
    ...
    def val_func_process_rgbX(self, input_data, input_modal_x1, input_modal_x2, device=None):
        input_data = np.ascontiguousarray(input_data[None, :, :, :], dtype=np.float32)
        input_data = torch.FloatTensor(input_data).cuda(device)
    
        input_modal_x1 = np.ascontiguousarray(input_modal_x1[None, :, :, :], dtype=np.float32)
        input_modal_x1 = torch.FloatTensor(input_modal_x1).cuda(device)
        if len(self.modality_x) == 1:
            input_modal_x2 = None
        elif len(self.modality_x) == 2:
            input_modal_x2 = np.ascontiguousarray(input_modal_x2[None, :, :, :], dtype=np.float32)
            input_modal_x2 = torch.FloatTensor(input_modal_x2).cuda(device)
        else:
            raise NotImplementedError
        ...
        with torch.cuda.device(input_data.get_device()):
            ...
            # self.val_func.to(input_data.get_device())
            with torch.no_grad():
                score = self.val_func(input_data, input_modal_x1, input_modal_x2)
                score = score[0]
                if self.is_flip:
                    input_data = input_data.flip(-1)
                    input_modal_x1 = input_modal_x1.flip(-1)
                    input_modal_x2 = input_modal_x2.flip(-1)
                    score_flip = self.val_func(input_data, input_modal_x1, input_modal_x2)
                    score_flip = score_flip[0]
                    score += score_flip.flip(-1)
                score = torch.exp(score)
    ...
    def process_image_rgbX(self, img, modal_x1, modal_x2, crop_size=None):
        p_img = img
        p_modal_x1 = modal_x1
        p_modal_x2 = modal_x2
        ...
        p_img = normalize(p_img, self.norm_mean, self.norm_std)
        if len(modal_x1.shape) == 2:
            p_modal_x1 = normalize(p_modal_x1, 0, 1)
        else:
            p_modal_x1 = normalize(p_modal_x1, self.norm_mean, self.norm_std)
        if len(modal_x2.shape) == 2:
            p_modal_x2 = normalize(p_modal_x2, 0, 1)
        else:
        ...
        if crop_size is not None:
            ...
            p_modal_x1, _ = pad_image_to_shape(p_modal_x1, crop_size, cv2.BORDER_CONSTANT, value=0)
            p_modal_x2, _ = pad_image_to_shape(p_modal_x2, crop_size, cv2.BORDER_CONSTANT, value=0)
            p_img = p_img.transpose(2, 0, 1)
            if len(modal_x1.shape) == 2:
                p_modal_x1 = p_modal_x1[np.newaxis, ...]
            else:
                p_modal_x1 = p_modal_x1.transpose(2, 0, 1) # 3 H W
            if len(modal_x2.shape) == 2:
                p_modal_x2 = p_modal_x2[np.newaxis, ...]
            else:
                p_modal_x2 = p_modal_x2.transpose(2, 0, 1) # 3 H W    
            return p_img, p_modal_x1, p_modal_x2, margin
        ...
        return p_img, p_modal_x1, p_modal_x2