...
# Dataset config
"""Dataset Path"""
C.dataset_name = 'Structured3D'
C.dataset_path = osp.join(C.root, 'datasets', 'Structured3D-1K')
C.area = 'full'
C.lighting = 'rawlight'
C.rgb = f'{C.area}-rgb-{C.lighting}-1K'
C.ann = f'{C.area}-semantic-1K'
C.modality_x = [f'{C.area}-depth-1K'] # TODO: change to f'{C.area}-depth-1K', f'{C.area}-normal-1K'
...
if C.modality_x[0] == f'{C.area}-depth-1K':
    C.log_dir = osp.abspath(osp.join(C.root, 'workdirs', 'Structured3D_1024x512/log_' + C.dataset_name + '_' + C.backbone + '_DMLPDecoderV2_Depth'))
elif C.modality_x[0] == f'{C.area}-normal-1K':
    C.log_dir = osp.abspath(osp.join(C.root, 'workdirs', 'Structured3D_1024x512/log_' + C.dataset_name + '_' + C.backbone + '_DMLPDecoderV2_Normal'))
else:
