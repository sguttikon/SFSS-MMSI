...
C.modality_x = [f'{C.area}-depth-1K', f'{C.area}-normal-1K']
...
if C.modality_x[0] == f'{C.area}-depth-1K' and C.modality_x[1] == f'{C.area}-normal-1K':
    C.log_dir = osp.abspath(osp.join(C.root, 'workdirs', 'Structured3D_1024x512/log_' + C.dataset_name + '_' + C.backbone + '_DMLPDecoderV2_Depth_Normal'))
else:
